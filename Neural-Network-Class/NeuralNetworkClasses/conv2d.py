...
class conv2d_layer(nn.Module):
    ...
    def __init__(self, params, activation, weight_init, verbose=False, **options):
        ...
        try: 
            ...
            self.in_ch, self.out_ch, self.kernel_size, self.stride, self.act = params[0], params[1], 3, 1, activation
            print("The neuron was falsly initialized: params[2] does not exist! A kernel_size of 3x3 will be used.")
            
        self.conv = nn.Conv2d(self.in_ch, self.out_ch, self.kernel_size, self.stride, **options)
        
        ######## Initialize the weights #############
        weight_init(self.conv.weight)
        
        if verbose:
            print("Conv2D, Input channels: ", self.in_ch, ", Output channels: ", self.out_ch, ", Kernel size: ", self.kernel_size, ", Stride: ", self.stride, ", activation: ", self.act)
